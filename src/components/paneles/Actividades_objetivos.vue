            ...
            <div class="multi-select-container">
              <div class="select-all-option">
                <input 
                  ...
                  :checked="selectedObjects.length === objetivos.length" 
                  @change="toggleAllObjects"
                >
                <label for="select-all-processes">Seleccionar todos</label>
              </div>
              <div class="multi-select-options">
                <div v-for="objetivo in objetivos" :key="objetivo.id" class="checkbox-item">
                  <input 
                    type="checkbox" 
                  ...
                  >
                  <label :for="`objetivo-${objetivo.id}`">{{ objetivo.denominacion }}</label>
                </div>
              </div>
            </div>
          </div>
...
const objetivos = ref([]);
const selectedObjects = ref([]); // Array para almacenar múltiples procesos seleccionados
...
const toggleAllObjects = (event) => {
  if (event.target.checked) {
    // Seleccionar todos los procesos
    selectedObjects.value = objetivos.value.map(objetivo => objetivo.id);
  } else {
    // Deseleccionar todos
...
const getApiParams = () => {
  ...
  if (selectedObjects.value.length > 0) {
    params.objetivos = selectedObjects.value.join(',');
  } else {
    params.objetivos = '-1'; // Valor para "Todos"
  }
...
const fetchActivityCounts = async () => {
  ...
  try {
    ...
    const baseUrl = 'http://localhost:8001/v1/dashboard/cantidad_actividades_objetivos';
...
// Función para cargar los objetivos desde la API
const fetchObjetivos = async () => {
  try {
    const token = apiToken.value;
    const response = await axios.get('http://localhost:8001/v1/objetivo_estrategico/', {
      headers: {
        'Authorization': `Token ${token}`
      }
    });
